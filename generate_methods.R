...
generate_methods <- function(email_address = NULL, 
                             email_zip = FALSE){
  ...
  if(!is.null(email_address)){ #render runs as background job, so sends email immediately
...
generate_methods() #email_address = "matthew@hirschey.org"
