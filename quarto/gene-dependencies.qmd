...
message("you downloaded the data")
...
message("you correlated the data")

achilles_cor_raw_shave_long <- 
  achilles_cor_raw_shave_long %>%
  corrr::shave() %>%  #shave to reduce redundancies
  
message("you shaved the data")

achilles_cor_raw_shave_long <- 
  achilles_cor_raw_shave_long %>%
  dplyr::mutate(across(where(is.numeric), ~ round(., digits = 4))) %>% #reduce memory use points
  dplyr::rename(rowname = 1) %>% 
  tidyr::pivot_longer(-rowname, names_to = "achilles_gene", values_to = "achilles_correlation_raw") %>% #pivot both longer, but use shaved dataset to plot only one gene-gene pair
  dplyr::filter(!is.na(achilles_correlation_raw))

message("you pivoted the data")
